...
class VehicleController(Node):
    def __init__(self):
        ...
        self.plot_logger = CrossTrackPlotLogger(self, "circular_arc")
    ...
    def main_timer_callback(self):
        ...
        if self.have_goal_pose and self.have_vehicle_pose:
            L_wheelbase_m = self.get_parameter("L_wheelbase_m").value
            ...
            # Prevent divide-by-zero or extremely large steering commands
            if abs(arc_radius) < 1e-6:
                if arc_radius >= 0.0:
                    angle_rad = max_steering_rad
                else:
                    angle_rad = -max_steering_rad
            else:
                angle_rad = math.atan(L_wheelbase_m / arc_radius)
